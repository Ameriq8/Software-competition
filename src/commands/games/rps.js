...
module.exports = class TestCommand extends BaseCommand {
  ...
  async run(client, message, args) {
    ...
    else if (botChoice > userChoice || botChoice === 0 && userChoice === 2) result = '**SkullBot** wins!';

    else result = `**${message.member.displayName}** wins!`;

    const embed = new MessageEmbed()

      .setTitle(`${message.member.displayName} vs. SkullBot`)

      .addField('Your Choice:', res[userChoice], true)

      .addField('SkullBot\'s Choice', res[botChoice], true)
